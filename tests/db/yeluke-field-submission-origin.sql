...
SELECT throws_like('
    ...
    ', '%must state its origin%', 'a direct write with no request identity and no origin should be refused')
; SELECT results_eq('
        INSERT INTO data.assignment_field_submission
        ...
        RETURNING origin
    ', ARRAY['provisioning'], 'a direct write may state its own origin, which is what admin provision-repos does')
;
...
SELECT throws_like('
    ...
    ', '%must state its origin%', 'a write with a user id but no role should be refused rather than classified')
; SELECT results_eq('
        INSERT INTO data.assignment_field_submission
            (assignment_submission_id, assignment_field_slug, body, origin)
        VALUES (7370, ''roleless-url'', ''https://github.com/example/roleless'', ''import'')
        RETURNING origin
    ', ARRAY['import'], 'a write with a user id but no role may still state its own origin')
; SET LOCAL "request.jwt.claim.user_id" TO ''
; SELECT throws_like('
        INSERT INTO data.assignment_field_submission
            (assignment_submission_id, assignment_field_slug, body, origin)
        VALUES (7370, ''third-url'', ''https://example.com/nonsense'', ''whatever'')
    ', '%origin_is_known%', 'origin should be constrained to the known vocabulary')
;
...
SELECT results_eq('
        ...
        ORDER BY id
    ', ' VALUES (''submitted''::text, ''provisioning''::text) ', 'the submitted event should carry the origin of the row it describes')
;
...
; SELECT results_eq('
        ...
        RETURNING origin
    ', ARRAY['student'], 'a student naming origin = provisioning on INSERT should still land as student')
;
...
SELECT results_eq('
        ...
        WHERE assignment_submission_id = 7370 AND assignment_field_slug = ''repo-url''
        RETURNING origin
    ', ARRAY['provisioning'], 'a student naming a different origin on UPDATE should leave origin unchanged')
;
...
SELECT results_eq('
        ...
        WHERE assignment_submission_id = 7370 AND assignment_field_slug = ''repo-url''
        RETURNING origin
    ', ARRAY['provisioning'], 'an ordinary student edit should leave a provisioned origin in place')
;
...
SELECT results_eq('
        ...
        RETURNING origin
    ', ARRAY['provisioning'], 'a student upsert over a provisioned row should leave origin unchanged')
;
...
SELECT results_eq('
        ...
        RETURNING origin
    ', ARRAY['student', 'student'], 'every row of a multi-row student INSERT should be classified as student')
; SELECT results_eq('
        UPDATE api.assignment_field_submissions
        ...
        RETURNING origin
    ', ARRAY['student', 'student'], 'every row of a multi-row student UPDATE should keep the origin it had')
;
...
SELECT results_eq('
        ...
        ORDER BY id DESC
        LIMIT 1
    ', ' VALUES (''revised''::text, ''provisioning''::text, 1) ', 'revising a provisioned row should append a revised event that keeps the provisioning origin')
;
-- ---------------------------------------------- staff writes say so ---------
...
; SELECT results_eq('
        ...
        RETURNING origin
    ', ARRAY['staff'], 'a faculty write should be recorded as staff whatever origin the payload names')
;
-- A deletion event carries the origin the row had.
DELETE FROM api.assignment_field_submissions
WHERE
    assignment_submission_id = 7370
    AND assignment_field_slug = 'third-url'
; RESET role
; SELECT results_eq('
        SELECT event_type, origin
        FROM data.assignment_field_submission_event
        ...
        ORDER BY id DESC
        LIMIT 1
    ', ' VALUES (''deleted''::text, ''staff''::text) ', 'the deleted event should carry the origin the row had')
; SELECT *
FROM finish()
