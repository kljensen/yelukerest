...
-- ---------------------------------------------------------------------------
SELECT has_table('data', 'assignment_repository', 'assignment_repository table should record provisioned repositories')
; SELECT view_owner_is('api', 'assignment_repositories', 'api', 'api.assignment_repositories view should be owned by the api role, or its RLS policy never applies')
; SELECT table_privs_are('api', 'assignment_repositories', 'faculty', ARRAY['SELECT', 'INSERT', 'UPDATE', 'DELETE'], 'faculty should have CRUD on view "api.assignment_repositories"')
; SELECT table_privs_are('api', 'assignment_repositories', 'student', ARRAY['SELECT'], 'student should only be granted SELECT on view "api.assignment_repositories"')
; SELECT table_privs_are('api', 'assignment_repositories', 'ta', ARRAY['SELECT'], 'ta should only be granted SELECT on view "api.assignment_repositories"')
; SELECT table_privs_are('data', 'assignment_repository', 'faculty', ARRAY[]::text[], 'faculty should be granted nothing on "data.assignment_repository"')
; SELECT table_privs_are('data', 'assignment_repository', 'student', ARRAY[]::text[], 'student should be granted nothing on "data.assignment_repository"')
;
-- ---------------------------------------------------------------------------
...
; SELECT set_eq('SELECT provider_full_name FROM api.assignment_repositories', ARRAY['mgt-656/exam-1-alice', 'mgt-656/project-update-1-bright-fog', 'mgt-656/team-selection-abc123'], 'a student should see their own repositories and their current team''s, and nothing else')
; SELECT is_empty('
...
SELECT throws_ok('
        ...
        WHERE provider_repo_id = 200000001
    ', '42501', NULL, 'a student cannot repoint their own assignment at another repository')
; SELECT throws_ok('DELETE FROM api.assignment_repositories WHERE provider_repo_id = 200000001', '42501', NULL, 'a student cannot delete their own repository record')
; SET "request.jwt.claim.user_id" TO "2"
; SELECT set_eq('SELECT provider_full_name FROM api.assignment_repositories', ARRAY['mgt-656/exam-1-bde456', 'mgt-656/project-update-1-hazy-mountain'], 'the other student sees their own repositories, and the first student''s are not among them')
;
...
; SELECT is_empty('SELECT provider_full_name FROM api.assignment_repositories', 'a ta with no repositories of their own should see none')
; RESET role
;
