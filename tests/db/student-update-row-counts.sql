...
; SELECT set_eq('
    ...
    ', ARRAY['assignment_field_submissions'], 'students should only have UPDATE privileges on assignment field submissions')
...
; SELECT results_eq('
        ...
        FROM updated_rows
    ', ARRAY[1], 'student broad assignment update should affect only their own individual field submission')
; SELECT results_eq('
        WITH updated_rows AS (
            UPDATE api.assignment_field_submissions
        ...
        SELECT count(*)::integer
        FROM updated_rows
    ', ARRAY[2], 'student broad team update should affect exactly their writable team field submissions')
...
; SELECT results_eq('
        ...
        SELECT count(*)::integer
        FROM updated_rows
    ', ARRAY[0], 'student broad team update should affect zero rows for another team')
...
; SELECT results_eq('
        ...
        SELECT count(*)::integer
        FROM updated_rows
    ', ARRAY[1], 'TA assignment field update should use the same RLS-narrowed row count')
; SELECT *
FROM finish()
