...
; SELECT set_eq('
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
    ', ARRAY['data.assignment_field_submission', 'data.assignment_submission', 'data.engagement'], 'the set of base tables a student or TA may write is the one this bound was designed for')
;
...
SELECT
    "is"((
        ...
        FROM pg_trigger
        WHERE
            tgrelid = 'data.assignment_field_submission'::regclass
            AND NOT tgisinternal
            AND tgfoid = 'data.enforce_single_assignment_submission()'::regprocedure
    ), 3, 'assignment field submissions are held to one parent submission per statement')
;
-- The numbers, so a change to either is a deliberate edit to this file.
SELECT "is"(data.request_row_bound_default(), 64, 'the default bound is 64 rows per request')
; SELECT
    "is"((
        SELECT count(*)::int
        FROM pg_trigger
        WHERE
            tgrelid = 'data.assignment_submission'::regclass
            AND NOT tgisinternal
            AND tgfoid = 'data.enforce_request_row_bound()'::regprocedure
            AND pg_get_triggerdef(oid) LIKE '%enforce_request_row_bound(''4'')'
    ), 3, 'assignment submissions keep the stricter bound of 4 rows per request')
;
...
SELECT set_eq('
    ...
    ', ARRAY['enforce_request_row_bound', 'enforce_single_assignment_submission'], 'the row-bound trigger functions are security definers with a pinned search_path')
;
...
SELECT
    ...
    ), true, 'api.check_request_jwt starts a fresh row budget for every request')
;
...
SELECT
    "is"((
        ...
        WHERE
            n.nspname = 'request'
            AND p.proname = 'reset_row_bound_counters'
    ), 1, 'the budget reset exists outside the schema PostgREST exposes')
; SELECT *
FROM finish()
