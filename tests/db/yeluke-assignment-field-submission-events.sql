...
SELECT *
FROM no_plan()
; SELECT view_owner_is('api', 'assignment_field_submission_events', 'api', 'api.assignment_field_submission_events view should be owned by the api role')
; SELECT table_privs_are('api', 'assignment_field_submission_events', 'faculty', ARRAY['SELECT'], 'faculty should only be granted SELECT on api.assignment_field_submission_events')
; SELECT table_privs_are('api', 'assignment_field_submission_events', 'student', ARRAY[]::text[], 'students should have no privileges on api.assignment_field_submission_events')
; SELECT table_privs_are('api', 'assignment_field_submission_events', 'ta', ARRAY[]::text[], 'tas should have no privileges on api.assignment_field_submission_events')
;
...
SELECT throws_like('
        ...
        WHERE assignment_submission_id = 6001
        AND assignment_field_slug = ''audit-field''
    ', '%stale write rejected%', 'updates supplying a mismatched updated_at should be rejected as stale')
; SELECT throws_ok('
        UPDATE api.assignment_field_submissions
        SET body = ''stale draft'', updated_at = updated_at - interval ''1 hour''
        WHERE assignment_submission_id = 6001
        AND assignment_field_slug = ''audit-field''
    ', 'PT409', NULL, 'stale writes should be rejected with SQLSTATE PT409 (HTTP 409 through PostgREST)')
; SELECT lives_ok('
        UPDATE api.assignment_field_submissions
...
SELECT results_eq('
    ...
    ', ARRAY[true], 'API inserts should ignore client-supplied created_at values')
; SELECT results_eq('
        UPDATE api.assignment_field_submissions
        ...
        RETURNING created_at > ''2020-01-01''::timestamptz
    ', ARRAY[true], 'updates should not be able to rewrite created_at')
;
-- ---------------------------------------------------------------
...
; SELECT results_eq('
        ...
        ORDER BY id DESC
        LIMIT 1
    ', ' VALUES (''deleted''::text, ''delete''::text) ', 'deleting a field submission should append a deleted event')
;
-- ---------------------------------------------------------------
...
; SELECT throws_like(' SELECT count(*) FROM api.assignment_field_submission_events ', '%permission denied%', 'students should not be able to read assignment field submission events')
;
-- ---------------------------------------------------------------
-- The history table is append-only
-- ---------------------------------------------------------------
RESET role
; SELECT throws_like(' UPDATE data.assignment_field_submission_event SET body_length = 0 WHERE id = (SELECT max(id) FROM data.assignment_field_submission_event) ', '%append-only%', 'assignment field submission events should reject UPDATE')
; SELECT throws_like(' DELETE FROM data.assignment_field_submission_event WHERE id = (SELECT max(id) FROM data.assignment_field_submission_event) ', '%append-only%', 'assignment field submission events should reject DELETE')
; SELECT *
FROM finish()
