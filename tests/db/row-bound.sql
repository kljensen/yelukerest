...
INSERT INTO data.assignment (slug, points_possible, is_draft, is_team, title, body, closed_at)
SELECT
    'zz-sub-' || g, 10, false, false, 'parent bound fixture ' || g, '',
...
; SELECT throws_ok('
    ...
    ', 'PT400', NULL, 'one row past the bound, in the same request, is refused')
; SELECT
    "is"((
        SELECT count(*)::int
        FROM api.assignment_field_submissions
        WHERE assignment_slug = 'zz-bound'
    ), 64, 'the refused statement changed nothing: still 64 rows')
;
...
; SELECT throws_ok('
        ...
        WHERE assignment_slug = ''zz-bound''
    ', 'PT400', NULL, 'an unfiltered UPDATE over 65 rows is refused')
;
...
; SELECT throws_ok('
    ...
    ', 'PT400', NULL, 'a batch upsert is counted across both of its arms, not once per arm')
; SELECT
    "is"((
        SELECT count(*)::int
        FROM api.assignment_field_submissions
        WHERE
            assignment_slug = 'zz-bound'
            AND body = 'upserted'
    ), 0, 'the refused upsert wrote nothing')
;
...
; SELECT throws_ok('
    ...
    ', 'PT400', NULL, 'one statement may not span two assignment submissions')
; SELECT
    "is"((
        SELECT count(*)::int
        FROM api.assignment_field_submissions
        WHERE body = 'across-two-submissions'
    ), 0, 'the statement spanning two submissions changed nothing')
; SELECT request.reset_row_bound_counters()
; SELECT lives_ok('
...
; SELECT throws_ok('INSERT INTO api.assignment_submissions (assignment_slug) VALUES (''zz-sub-5'')', 'PT400', NULL, 'a fifth assignment submission in the same request is refused')
; SELECT
    "is"((
        SELECT count(*)::int
        FROM api.assignment_submissions
        WHERE assignment_slug LIKE 'zz-sub-%'
    ), 4, 'the refused parent insert changed nothing')
;
...
; SELECT throws_ok('
    ...
    ', 'PT400', NULL, 'the parent bound counts the whole request, not each statement')
;
...
; SELECT set_config('request.jwt.claim.role', '', false)
; SELECT set_config('request.jwt.claim.user_id', '', false)
; SELECT set_config('request.jwt.claims', json_build_object('role', 'student', 'user_id', 1, 'netid', 'abc123')::text, false)
; SET LOCAL role TO student
; SELECT request.reset_row_bound_counters()
; SELECT throws_ok('
        UPDATE api.assignment_field_submissions SET body = ''json-claims''
        WHERE assignment_slug = ''zz-bound''
    ', 'PT400', NULL, 'the bound reads the JSON claims GUC as well as the individual claim GUCs')
;
...
RESET role
; SELECT set_config('request.jwt.claims', '', false)
; CREATE FUNCTION api.zz_two_statement_write() RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, data, pg_temp AS $$
BEGIN
...
; SET "request.jwt.claim.user_id" TO "1"
; SELECT request.reset_row_bound_counters()
; SELECT throws_ok('SELECT api.zz_two_statement_write()', 'PT400', NULL, 'a multi-statement SECURITY DEFINER function does not reset the budget')
; SELECT
    "is"((
        SELECT count(*)::int
        FROM api.assignment_field_submissions
        WHERE body = 'rpc-first'
    ), 0, 'the refused function call rolled back its earlier statement too')
;
-- ------------------------------------------------------------- the TA -------
RESET role
...
; SELECT request.reset_row_bound_counters()
; SELECT throws_ok('UPDATE api.engagements SET participation = ''led'' WHERE meeting_slug = ''intro''', 'PT400', NULL, 'a TA marking a roster larger than the bound in one statement is refused')
; SELECT request.reset_row_bound_counters()
; SELECT lives_ok('
...
; SELECT lives_ok('DELETE FROM api.engagements WHERE meeting_slug = ''intro''', 'faculty bulk deletes -- what sync_meetings does -- are unaffected')
;
