...
; SELECT throws_like('
    ...
    ', '%violates check constraint%', 'assignment_field_submissions must match the assignment_field pattern (negative case)')
; SELECT lives_ok('
      INSERT INTO
        api.assignment_field_submissions (assignment_submission_id,assignment_field_slug,assignment_slug,body,origin)
...
; SELECT throws_like('
    ...
    ', '%violates check constraint%', 'assignment_field_submissions require a body that is a URL if is_url is TRUE (negative case)')
; SELECT lives_ok('
      INSERT INTO
        api.assignment_field_submissions (assignment_submission_id,assignment_field_slug,assignment_slug,body,origin)
