...
-- An import ledger with before and after images, and a faculty summary
-- (issue #390).
--
-- With TAs recording grades (#389), faculty need to see at a glance what each
-- import did, and later to undo one (#391). The grade event stream cannot
-- carry that on its own: data.record_quiz_grade_event writes the row state
-- after an insert or update, so a 'corrected' event has the new points but
-- not the prior ones; the quiz submissions the import creates and the
-- attendance it promotes are in no stream at all; and import_id was whatever
-- the caller sent, so two runs could share one and a summary grouped from
-- events would miss an import whose every row was unchanged.
--
-- So api.import_quiz_results now writes a ledger in the same transaction as
-- the grades, dry runs included:
--   * data.quiz_grade_import, one header per call: a generated execution id,
--     the caller's label (the old p_import_id, kept but never used as
--     identity), actor, role, reason, dry-run flag, and the counts the call
--     returned;
--   * data.quiz_grade_import_item, one row per result: the mutation kind, the
--     grade before and after, whether a submission was created, and the
--     attendance before and after. A dry run records what would have
--     happened.
-- The generated execution id is what the function now returns as import_id
-- and what it stamps on data.quiz_grade_event.import_id, so events link to
-- the header. Both tables are append-only: INSERT for quiz_importer under a
-- policy requiring a faculty or TA claim, SELECT for api, and no UPDATE or
-- DELETE for anyone but the migrator. api.quiz_grade_imports lists the
-- headers to faculty with per-kind counts from the items.
--
-- The ledger follows the event tables in carrying quiz_id and user_id as
-- plain integers rather than foreign keys: history has to outlive the rows it
-- describes, and a reversal of an import whose quiz was since deleted is a
-- no-op, not a broken reference. The same goes for the actor.
-- ---------------------------------------------------------------------------
-- Storage
-- ---------------------------------------------------------------------------
-- reverts_import_id is for #391: the import this one undid, if it is a
-- reversal.
CREATE TABLE data.quiz_grade_import (
    id uuid PRIMARY KEY,
    label text,
    actor_user_id int NOT NULL,
    actor_role text NOT NULL CHECK (actor_role IN ('faculty', 'ta')),
    reason text,
    ...
    submission_created_count int NOT NULL CHECK (submission_created_count >= 0),
    attendance_inserted int NOT NULL CHECK (attendance_inserted >= 0),
    attendance_updated int NOT NULL CHECK (attendance_updated >= 0),
    attendance_unchanged int NOT NULL CHECK (attendance_unchanged >= 0),
    reverts_import_id uuid REFERENCES data.quiz_grade_import (id),
    CONSTRAINT quiz_grade_import_cannot_revert_itself CHECK (reverts_import_id IS DISTINCT FROM id)
)
;
-- tests/db/foreign-key-indexes.sql requires an index led by every referencing
-- column. The actor index serves the summary's likely filter.
CREATE INDEX quiz_grade_import_reverts_import_id_idx
ON data.quiz_grade_import USING btree (reverts_import_id)
; CREATE INDEX quiz_grade_import_actor_user_id_idx
ON data.quiz_grade_import USING btree (actor_user_id)
; ALTER TABLE data.quiz_grade_import
    OWNER TO yelukerest_migrator
; COMMENT ON TABLE data.quiz_grade_import IS 'One row per call of api.import_quiz_results, dry runs included. The id is generated per execution; label is the caller''s p_import_id and is not unique. Append-only.'
;
-- The before image is NULL exactly when the row was an insert; an unchanged
-- row's after image equals its before image; an update's differs; attendance
-- is kept, or a missing or absent row is promoted to attended, the only
-- attendance change an import makes. Declared, so a later writer cannot
-- record a contradiction.
CREATE TABLE data.quiz_grade_import_item (
    import_id uuid NOT NULL REFERENCES data.quiz_grade_import (id),
    quiz_id int NOT NULL,
    user_id int NOT NULL,
    mutation text NOT NULL CHECK (mutation IN ('insert', 'update', 'unchanged')),
    points_before real,
    points_possible_before smallint,
    description_before text,
    points_after real NOT NULL,
    points_possible_after smallint NOT NULL,
    description_after text,
    submission_created boolean NOT NULL,
    attendance_before data.participation_enum,
    attendance_after data.participation_enum,
    PRIMARY KEY (import_id, quiz_id, user_id),
    CONSTRAINT quiz_grade_import_item_before_image_iff_existing CHECK ((mutation = 'insert'
    AND points_before IS NULL
    AND points_possible_before IS NULL
    AND description_before IS NULL) OR (mutation <> 'insert'
    AND points_before IS NOT NULL
    AND points_possible_before IS NOT NULL)),
    CONSTRAINT quiz_grade_import_item_unchanged_means_equal CHECK (mutation <> 'unchanged' OR (points_before IS NOT DISTINCT FROM points_after
    AND points_possible_before IS NOT DISTINCT FROM points_possible_after
    AND description_before IS NOT DISTINCT FROM description_after)),
    CONSTRAINT quiz_grade_import_item_update_means_different CHECK (mutation <> 'update' OR (points_before, description_before) IS DISTINCT FROM (points_after, description_after)),
    CONSTRAINT quiz_grade_import_item_attendance_promoted_or_kept CHECK (attendance_after IS NOT DISTINCT FROM attendance_before OR ((attendance_before IS NULL OR attendance_before = 'absent'::data.participation_enum)
    AND attendance_after = 'attended'::data.participation_enum))
)
; ALTER TABLE data.quiz_grade_import_item
    OWNER TO yelukerest_migrator
; COMMENT ON TABLE data.quiz_grade_import_item IS 'One row per result in a quiz grade import: the grade before and after, whether its submission was created, and attendance before and after. A dry run records what would have happened. Append-only.'
;
-- ---------------------------------------------------------------------------
-- Privileges: append-only
-- ---------------------------------------------------------------------------
-- The import writes the ledger as quiz_importer, under a policy that requires
-- the same claim its other policies do. api reads it for the faculty view.
-- Nobody but the owner holds UPDATE or DELETE, and the owner is the migrator,
-- which never runs a request.
ALTER TABLE data.quiz_grade_import
    ENABLE ROW LEVEL SECURITY
; ALTER TABLE data.quiz_grade_import_item
    ENABLE ROW LEVEL SECURITY
; GRANT insert ON data.quiz_grade_import, data.quiz_grade_import_item TO quiz_importer
; CREATE POLICY quiz_importer_insert ON data.quiz_grade_import FOR INSERT TO quiz_importer WITH CHECK (request.user_role() IN ('faculty', 'ta'))
; CREATE POLICY quiz_importer_insert ON data.quiz_grade_import_item FOR INSERT TO quiz_importer WITH CHECK (request.user_role() IN ('faculty', 'ta'))
; GRANT select ON data.quiz_grade_import, data.quiz_grade_import_item TO api
; CREATE POLICY quiz_grade_import_access_policy ON data.quiz_grade_import FOR SELECT TO api USING (request.user_role() = 'faculty')
; CREATE POLICY quiz_grade_import_item_access_policy ON data.quiz_grade_import_item FOR SELECT TO api USING (request.user_role() = 'faculty')
;
-- ---------------------------------------------------------------------------
-- The import, now writing its ledger
-- ---------------------------------------------------------------------------
-- The body 01a0b1c5-add-ta-quiz-import shipped, with these changes: the
-- execution id is generated up front and is what import_id returns and what
-- the grade events carry; the caller's p_import_id is kept as the label; the
-- grade and engagement rows in the batch are locked before they are read;
-- the batch is planned once, as the ledger items, and the counts, the
-- header, the items and the grade writes all come from that plan; the
-- ledger is written once the batch has passed every check and before the
-- dry-run return; and a real run checks that what it wrote is what the
-- ledger says it planned. Same signature, same validation order, same
-- counts. CREATE OR REPLACE keeps the owner, quiz_importer, and the execute
-- grants.
...
-- The faculty summary
-- ---------------------------------------------------------------------------
-- Driven by the header, so an import whose every row was unchanged is still
-- listed with its actor and reason. The grade counts come from the items,
-- which is the record of what was done row by row; the header carries the
-- same numbers, and the test suite holds the two to each other. Attendance
-- counts come from the header because an item cannot tell an import that did
-- not ask for attendance from one that found everyone already present.
...
; ALTER VIEW api.quiz_grade_imports
    OWNER TO api
; COMMENT ON VIEW api.quiz_grade_imports IS 'Every call of api.import_quiz_results, dry runs included, newest first, with what it did. Faculty only.'
...
; COMMENT ON COLUMN api.quiz_grade_imports.label IS 'The caller''s p_import_id, if any. A label, not an identity: two calls may share one'
...
; COMMENT ON COLUMN api.quiz_grade_imports.attendance_inserted IS 'Engagement rows created saying attended'
; COMMENT ON COLUMN api.quiz_grade_imports.attendance_updated IS 'Engagement rows promoted from absent to attended'
; COMMENT ON COLUMN api.quiz_grade_imports.attendance_unchanged IS 'People in the batch whose attendance was already recorded, when attendance was asked for'
; GRANT select ON api.quiz_grade_imports TO faculty
;
-- ---------------------------------------------------------------------------
-- Compatibility
-- ---------------------------------------------------------------------------
-- admin_api_version 12: import_id in the reply is the generated execution
-- id, and api.quiz_grade_imports exists. The reply's shape is unchanged, so
-- schema_compatibility_version stays 7.
CREATE OR REPLACE VIEW api.platform_version AS
    SELECT
        'yelukerest'::text AS platform,
        1::int AS platform_compatibility_version,
        7::int AS schema_compatibility_version, 12::int AS admin_api_version
