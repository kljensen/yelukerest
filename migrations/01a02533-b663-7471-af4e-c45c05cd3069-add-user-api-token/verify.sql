...
; SELECT
    1 / (
        ...
        WHERE
            table_schema = 'data'
            AND table_name = 'user_api_token'
            AND column_name IN ('id', 'user_id', 'token_prefix', 'token_hash', 'name', 'scopes', 'created_at', 'expires_at', 'last_used_at', 'revoked_at')
    )::int
;
...
WHERE
    ix.indrelid = 'data.user_api_token'::regclass
    AND ix.indpred IS NULL
    AND (
        SELECT attname
        FROM pg_attribute
        WHERE
            attrelid = ix.indrelid
...
; SELECT
    1 / (
        ...
        WHERE
            n.nspname = 'auth'
            AND p.proname = 'sign_user_jwt_with_scopes'
            AND p.prosecdef
    )::int
; SELECT
    1 / (
        SELECT (NOT has_function_privilege('public', 'api.exchange_user_api_token(text)', 'EXECUTE'))::int
    )
;
...
SELECT
    1 / (
        ...
        FROM pg_constraint
        WHERE
            conrelid = 'data.user_api_token'::regclass
            AND contype = 'c'
    )
;
...
SELECT
    1 / (
        SELECT (schema_compatibility_version >= 7)::int
