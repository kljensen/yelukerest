...
SELECT
    1 / (
        ...
        WHERE
            ...
            AND (a.attname, format_type(a.atttypid, a.atttypmod), a.attnotnull) IN (('token', 'text', true), ('data', 'bytea', true), ('expiry', 'timestamp with time zone', true))
    )
;
...
SELECT
    1 / (
        ...
        FROM pg_index i
        WHERE
            i.indrelid = 'data.authapp_session'::regclass
            AND i.indisprimary
            AND i.indkey::text = (
                SELECT a.attnum::text
                FROM pg_attribute a
                WHERE
                    a.attrelid = 'data.authapp_session'::regclass
                    AND a.attname = 'token'
            )
    )
;
...
SELECT
    1 / (has_table_privilege('authapp', 'data.authapp_session', 'SELECT')
...
SELECT
    1 / (
        SELECT
            COALESCE(array_agg(a.privilege_type ORDER BY a.privilege_type) = ARRAY['DELETE', 'INSERT', 'SELECT', 'UPDATE']
            AND bool_and(NOT a.is_grantable), false)::int
        FROM
            pg_class c
            CROSS JOIN LATERAL aclexplode(c.relacl) a
            JOIN pg_roles g ON g.oid = a.grantee
        WHERE
            c.oid = 'data.authapp_session'::regclass
...
SELECT
    1 / (
        SELECT COALESCE(array_agg(DISTINCT g.rolname::text) = ARRAY['authapp', 'yelukerest_migrator'], false)::int
        FROM
            pg_class c
            CROSS JOIN LATERAL aclexplode(c.relacl) a
...
SELECT
    1 / (
        ...
        FROM pg_roles r
        WHERE
            r.rolname NOT IN ('authapp', 'yelukerest_migrator')
            AND NOT r.rolsuper
            AND r.rolname NOT LIKE E'pg\\_%'
            AND (has_table_privilege(r.oid, 'data.authapp_session', 'SELECT') OR has_table_privilege(r.oid, 'data.authapp_session', 'INSERT') OR has_table_privilege(r.oid, 'data.authapp_session', 'UPDATE') OR has_table_privilege(r.oid, 'data.authapp_session', 'DELETE'))
    )
;
