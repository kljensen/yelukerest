...
--
-- Drops the reversal, takes back what quiz_importer gained for it, puts the
-- engagement UPDATE policy, the import and the summary view back as 01a0b1c5
-- and 01a0b208 left them, and restores the ledger's shape. The item rules that
-- come back forbid what only a reversal writes, and the ledger is
-- append-only audit evidence of grades that were actually moved, so this
-- refuses to run once any reversal has been recorded rather than delete the
-- record of it. Revert is for a deploy that has not yet been used.
DO $$
DECLARE
...
BEGIN
    ...
    WHERE reverts_import_id IS NOT NULL;

    IF reversal_count > 0 THEN
        RAISE EXCEPTION 'add-quiz-import-revert cannot be reverted: % reversal header(s) exist in data.quiz_grade_import, and the ledger is append-only audit evidence', reversal_count
            USING HINT = 'a corrective migration, not a revert, is the way forward once api.revert_quiz_import has been used';
    END IF;
END;
$$
; DROP FUNCTION api.revert_quiz_import(uuid, text, boolean)
; DROP VIEW api.quiz_grade_imports
; DROP POLICY quiz_importer_delete ON data.quiz_grade
; REVOKE delete ON data.quiz_grade FROM quiz_importer
; DROP POLICY quiz_importer_delete ON data.engagement
; REVOKE delete ON data.engagement FROM quiz_importer
; DROP POLICY quiz_importer_select ON data.quiz_grade_import
; DROP POLICY quiz_importer_select ON data.quiz_grade_import_item
; DROP POLICY quiz_importer_select ON data.quiz_grade_event
; REVOKE select ON data.quiz_grade_import, data.quiz_grade_import_item, data.quiz_grade_event FROM quiz_importer
...
; ALTER TABLE data.quiz_grade_import_item
    DROP CONSTRAINT quiz_grade_import_item_attendance_promoted_kept_or_restored,
    ADD CONSTRAINT quiz_grade_import_item_attendance_promoted_or_kept CHECK (attendance_after IS NOT DISTINCT FROM attendance_before OR ((attendance_before IS NULL OR attendance_before = 'absent'::data.participation_enum)
    AND attendance_after = 'attended'::data.participation_enum)),
    DROP CONSTRAINT quiz_grade_import_item_images_match_mutation,
    ADD CONSTRAINT quiz_grade_import_item_before_image_iff_existing CHECK ((mutation = 'insert'
    ...
    ADD CONSTRAINT quiz_grade_import_item_mutation_check CHECK (mutation IN ('insert', 'update', 'unchanged')),
    ALTER COLUMN points_after SET NOT NULL,
    ALTER COLUMN points_possible_after SET NOT NULL
...
; ALTER TABLE data.quiz_grade_import
    DROP deleted_count,
    DROP attendance_deleted
;
-- The import as 01a0b208 defined it, locking in payload order.
...
;
-- The summary as 01a0b208 defined it.
...
;
-- Back to the compatibility versions 01a0b208-add-quiz-grade-import-ledger
-- reported.
