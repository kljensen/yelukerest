...
--
CREATE FUNCTION api.check_request_jwt() RETURNS void LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO pg_catalog, api, settings, request, pg_temp AS $$
DECLARE
    claims jsonb;
    claim_role text;
    claim_issuer text;
...
--
CREATE FUNCTION pgjwt.sign(payload pg_catalog.json, secret text, algorithm text = 'HS256'::text) RETURNS text LANGUAGE sql AS $$
WITH
  header AS (
...
$$
; ALTER FUNCTION pgjwt.sign(payload pg_catalog.json, secret text, algorithm text) OWNER TO yelukerest_migrator
...
; ALTER TABLE settings.secrets
    OWNER TO yelukerest_migrator
;
--
-- Name: get(text); Type: FUNCTION; Schema: settings; Owner: cluster_admin
--
CREATE FUNCTION settings.get(text) RETURNS text LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO pg_catalog, settings, pg_temp RETURN (
...
--
CREATE FUNCTION auth.sign_jwt(user_id int, role data.user_role) RETURNS text LANGUAGE sql SECURITY DEFINER SET search_path TO pg_catalog, auth, settings, pgjwt, pg_temp RETURN pgjwt.sign(json_build_object('iss', settings.get('jwt_issuer'::text), 'aud', settings.get('jwt_audience'::text), 'sub', 'user:'::text || user_id::text, 'user_id', user_id, 'role', role::text, 'iat', extract ('epoch' FROM now())::int, 'nbf', extract ('epoch' FROM now())::int, 'jti', public.gen_random_uuid()::text, 'exp', extract ('epoch' FROM now())::int + settings.get('jwt_lifetime'::text)::int), settings.get('jwt_secret'::text))
; ALTER FUNCTION auth.sign_jwt(user_id int, role data.user_role) OWNER TO yelukerest_migrator
;
--
-- Name: jwt_claim(text); Type: FUNCTION; Schema: request; Owner: cluster_admin
--
CREATE FUNCTION request.jwt_claim(claim text) RETURNS text LANGUAGE sql STABLE RETURN COALESCE(NULLIF(current_setting('request.jwt.claim.'::text || claim, true), ''::text), NULLIF((NULLIF(current_setting('request.jwt.claims'::text, true), ''::text))::pg_catalog.json ->> claim, ''::text))
...
--
CREATE FUNCTION request.app_name() RETURNS text LANGUAGE sql STABLE RETURN request.jwt_claim('app_name'::text)
...
--
CREATE FUNCTION request.user_id_as_text() RETURNS text LANGUAGE sql STABLE RETURN request.jwt_claim('user_id'::text)
...
--
CREATE FUNCTION request.user_id() RETURNS int LANGUAGE sql STABLE RETURN CASE request.user_id_as_text()
    WHEN ''::text THEN 0
    ELSE request.user_id_as_text()::int
END
...
--
CREATE FUNCTION request.user_role() RETURNS text LANGUAGE sql STABLE RETURN request.jwt_claim('role'::text)
...
CREATE TABLE data."user" (
    ...
    CONSTRAINT user_email_check CHECK (email ~ E'^[a-zA-Z0-9.!#$%&''*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$'::text
    ...
    CONSTRAINT user_netid_check CHECK (netid ~ '^[a-z]+[0-9]+$'::text
    AND char_length(netid) < 10),
    CONSTRAINT user_nickname_check CHECK (nickname ~ E'^[\\w]{2,20}-[\\w]{2,20}$'::text
    ...
    SELECT
        CASE
            WHEN
                ...
                AND (request.user_role() = 'faculty'::text OR request.user_id() = id OR (request.user_role() = 'app'::text
                AND request.app_name() = 'authapp'::text)) THEN auth.sign_jwt(id, role)
            ELSE NULL::text
        END AS jwt,
        id, email, netid, name, lastname, organization, known_as, nickname,
        role, created_at, updated_at, team_nickname
    FROM data."user"
; ALTER VIEW api.user_jwts
    OWNER TO api
;
--
...
--
CREATE FUNCTION api.issue_user_jwt(requested_netid text) RETURNS SETOF api.user_jwts LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO pg_catalog, api, request, pg_temp BEGIN ATOMIC
    SELECT
        user_jwts.jwt, user_jwts.id, user_jwts.email, user_jwts.netid,
        user_jwts.name, user_jwts.lastname, user_jwts.organization,
        user_jwts.known_as, user_jwts.nickname, user_jwts.role,
        user_jwts.created_at, user_jwts.updated_at, user_jwts.team_nickname
    FROM api.user_jwts
    WHERE
        user_jwts.netid = issue_user_jwt.requested_netid
        AND request.user_role() = 'app'::text
        AND request.app_name() = 'authapp'::text
; END
...
--
CREATE FUNCTION api.issue_user_jwt_for_mcp(p_netid text, p_scopes text[], p_external jsonb = NULL::jsonb) RETURNS TABLE (jwt text, user_id int, netid text, role text) LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, api, auth, data, request, settings, pg_temp AS $$
DECLARE
...
--
CREATE FUNCTION api.record_mcp_grant_revocation(netid text, client_id text, client_name text = NULL::text, scopes text = NULL::text) RETURNS TABLE (revoked_at timestamp with time zone) LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, api, auth, data, request, settings, pg_temp AS $$
DECLARE
    target_user data."user"%ROWTYPE;
...
--
CREATE FUNCTION api.sync_assignments(p_assignments jsonb, p_delete_missing boolean = false, p_dry_run boolean = false) RETURNS TABLE (inserted_count int, updated_count int, unchanged_count int, deleted_count int, field_inserted_count int, field_updated_count int, field_unchanged_count int, field_deleted_count int, dry_run boolean) LANGUAGE plpgsql AS $$
DECLARE
    input_count integer;
...
--
CREATE FUNCTION api.sync_meetings(p_meetings jsonb) RETURNS TABLE (inserted_count int, updated_count int, unchanged_count int, deleted_count int) LANGUAGE plpgsql AS $$
DECLARE
    input_count integer;
    duplicate_slug text;
BEGIN
...
--
CREATE FUNCTION auth.sign_mcp_user_jwt(user_id int, role data.user_role, netid text, scopes text, jti text) RETURNS text LANGUAGE sql SECURITY DEFINER SET search_path TO pg_catalog, auth, settings, pgjwt, pg_temp RETURN pgjwt.sign(json_build_object('iss', settings.get('jwt_issuer'::text), 'aud', json_build_array(settings.get('jwt_audience'::text), COALESCE(settings.get('jwt_mcp_audience'::text), 'yelukerest-mcp'::text)), 'sub', 'user:'::text || user_id::text, 'user_id', user_id, 'role', role::text, 'netid', netid, 'scopes', scopes, 'iat', extract ('epoch' FROM now())::int, 'nbf', extract ('epoch' FROM now())::int, 'jti', jti, 'exp', extract ('epoch' FROM now())::int + 600), settings.get('jwt_secret'::text))
; ALTER FUNCTION auth.sign_mcp_user_jwt(user_id int, role data.user_role, netid text, scopes text, jti text) OWNER TO yelukerest_migrator
;
--
-- Name: assignment_field_submission_is_writable_by_current_user(integer); Type: FUNCTION; Schema: data; Owner: cluster_admin
--
CREATE FUNCTION data.assignment_field_submission_is_writable_by_current_user(the_assignment_submission_id int) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
    RETURN EXISTS (
        SELECT ass_sub.id
...
END;
$$
; ALTER FUNCTION data.assignment_field_submission_is_writable_by_current_user(the_assignment_submission_id int) OWNER TO yelukerest_migrator
;
--
-- Name: clean_user_fields(); Type: FUNCTION; Schema: data; Owner: cluster_admin
--
CREATE FUNCTION data.clean_user_fields() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
--
CREATE FUNCTION data.ensure_student_engagement_rows() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
    IF (
...
--
CREATE FUNCTION data.fill_assignment_field_submission_defaults() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
...
--
CREATE FUNCTION data.fill_assignment_grade_defaults() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
...
--
CREATE FUNCTION data.fill_assignment_grade_exception_defaults() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
...
--
CREATE FUNCTION data.fill_assignment_submission_defaults() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
    -- Set default is_team from assignment table
    IF (NEW.is_team IS NULL) THEN
        SELECT is_team INTO NEW.is_team
...
--
CREATE FUNCTION data.fill_grade_defaults() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    NEW.updated_at = current_timestamp;
    RETURN NEW;
END;
...
--
CREATE FUNCTION data.fill_grade_snapshot_defaults() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    NEW.updated_at = current_timestamp;
    RETURN NEW;
END;
...
--
CREATE FUNCTION data.fill_quiz_grade_defaults() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
...
--
CREATE FUNCTION data.fill_quiz_submission_defaults() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
--
CREATE FUNCTION data.fill_user_secret_defaults() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    NEW.updated_at = current_timestamp;
    RETURN NEW;
END;
...
--
CREATE FUNCTION data.mcp_grant_revocation_is_append_only() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
--
CREATE FUNCTION data.prevent_assignment_field_submission_event_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
--
CREATE FUNCTION data.prevent_grade_event_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
--
CREATE FUNCTION data.prevent_mcp_jwt_mint_event_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
--
CREATE FUNCTION data.quiz_set_defaults() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
...
--
CREATE FUNCTION data.record_assignment_field_submission_event() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
DECLARE
...
--
CREATE FUNCTION data.record_assignment_grade_event() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
DECLARE
...
--
CREATE FUNCTION data.record_grade_event() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
DECLARE
...
--
CREATE FUNCTION data.record_quiz_grade_event() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
DECLARE
...
--
CREATE FUNCTION data.refresh_assignment_submission_participants() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO data, pg_temp AS $$
BEGIN
...
--
CREATE FUNCTION data.text_is_url(text) RETURNS boolean LANGUAGE sql STABLE RETURN char_length($1) <= 2048
AND $1 ~* '^https?://[a-z0-9]+'::text
...
--
CREATE FUNCTION data.text_matches(text, text) RETURNS boolean LANGUAGE sql STABLE RETURN $1 ~ (('^(?:'::text || $2) || ')$'::text)
...
--
CREATE FUNCTION data.update_updated_at_column() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    NEW.updated_at = current_timestamp;
    RETURN NEW;
END;
...
--
CREATE FUNCTION pgjwt.algorithm_sign(signables text, secret text, algorithm text) RETURNS text LANGUAGE sql AS $$
WITH
  alg AS (
    SELECT CASE
...
--
CREATE FUNCTION pgjwt.url_decode(data text) RETURNS bytea LANGUAGE sql AS $$
...
--
CREATE FUNCTION pgjwt.url_encode(data bytea) RETURNS text LANGUAGE sql AS $$
...
--
CREATE FUNCTION pgjwt.verify(token text, secret text, algorithm text = 'HS256'::text) RETURNS TABLE (header pg_catalog.json, payload pg_catalog.json, valid boolean) LANGUAGE sql AS $$
  SELECT
...
--
CREATE FUNCTION settings.set(text, text) RETURNS void LANGUAGE sql SECURITY DEFINER SET search_path TO pg_catalog, settings, pg_temp BEGIN ATOMIC
...
CREATE TABLE data.artifact (
    ...
    AND content_length <= '5368709120'::bigint)),
    CONSTRAINT artifact_content_type_check CHECK (content_type IS NULL OR (char_length(content_type) <= 255
    AND content_type ~ '^[A-Za-z0-9][A-Za-z0-9!#$&^_.+-]*/[A-Za-z0-9][A-Za-z0-9!#$&^_.+-]*$'::text)),
    CONSTRAINT artifact_description_check CHECK (char_length(description) < 1000),
    CONSTRAINT artifact_slug_check CHECK (slug ~ '^[a-z0-9][a-z0-9_-]+[a-z0-9]$'::text
...
CREATE VIEW api.artifacts AS
    SELECT
        id, user_id, quiz_id, slug, title, description, url, storage_uri,
        content_type, content_length, checksum_sha256, is_user_visible,
        created_at, updated_at
    FROM data.artifact
; ALTER VIEW api.artifacts
    OWNER TO api
;
--
...
CREATE TABLE data.assignment_field_submission_event (
    ...
    assignment_submission_id int CONSTRAINT assignment_field_submission_e_assignment_submission_id_not_null NOT NULL,
...
CREATE VIEW api.assignment_field_submission_events AS
    SELECT
        id, event_type, operation, assignment_submission_id,
        assignment_field_slug, assignment_slug, body_sha256, body_length,
        submitter_user_id, submission_created_at, submission_updated_at,
...
CREATE TABLE data.assignment_field_submission (
    ...
    body text NOT NULL,
    submitter_user_id int DEFAULT request.user_id() NOT NULL,
    created_at timestamp with time zone DEFAULT current_timestamp NOT NULL,
    updated_at timestamp with time zone DEFAULT current_timestamp NOT NULL,
...
CREATE VIEW api.assignment_field_submissions AS
    SELECT
        ...
        assignment_field_is_url, assignment_field_pattern, body,
        submitter_user_id, created_at, updated_at
...
CREATE TABLE data.assignment (
    ...
    CONSTRAINT assignment_slug_check CHECK (slug ~ '^[a-z0-9-]+$'::text
...
CREATE TABLE data.assignment_field (
    ...
    CONSTRAINT assignment_field_slug_check CHECK (slug ~ '^[a-z0-9-]+$'::text
    AND char_length(slug) < 30),
    CONSTRAINT pattern_matches_example CHECK (data.text_matches(example, pattern)),
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at),
    CONSTRAINT url_matches_example CHECK (is_url IS FALSE OR (is_url IS TRUE
    AND data.text_is_url(example))),
    CONSTRAINT url_not_multiline CHECK (NOT (is_url
    AND is_multiline))
)
; ALTER TABLE data.assignment_field
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.assignment_fields WITH (security_barrier=true) AS
    ...
    WHERE
        request.user_role() = 'faculty'::text OR EXISTS (
            SELECT 1
            FROM data.assignment assignment
            WHERE
                assignment.slug = field.assignment_slug
...
CREATE TABLE data.assignment_grade (
    ...
    CONSTRAINT points_in_range CHECK (points >= 0::double precision
    AND points <= points_possible::double precision),
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at)
)
; ALTER TABLE data.assignment_grade
    OWNER TO yelukerest_migrator
;
--
...
CREATE TABLE data.assignment_submission (
    ...
    CONSTRAINT submitter_matches_user_id CHECK (is_team OR (NOT is_team
    AND user_id = submitter_user_id)),
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at)
)
; ALTER TABLE data.assignment_submission
    OWNER TO yelukerest_migrator
;
--
...
CREATE TABLE data.assignment_submission_participant (
    assignment_submission_id int CONSTRAINT assignment_submission_partici_assignment_submission_id_not_null NOT NULL,
    user_id int NOT NULL,
    created_at timestamp with time zone DEFAULT current_timestamp NOT NULL
)
; ALTER TABLE data.assignment_submission_participant
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.assignment_grade_distributions AS
    WITH included_scores AS (
        SELECT
            assignment.slug AS assignment_slug, assignment.points_possible,
            COALESCE(assignment_grade.points, 0::real) AS points
        FROM
            data.assignment
            JOIN data."user" student ON student.role = 'student'::data.user_role
            LEFT JOIN data.assignment_submission sub ON sub.assignment_slug = assignment.slug
            AND NOT sub.is_team
        ...
        UNION ALL
        SELECT
            sub.assignment_slug, assignment.points_possible,
            COALESCE(assignment_grade.points, 0::real) AS points
        FROM
            data.assignment_submission sub
            JOIN data.assignment ON assignment.slug = sub.assignment_slug
            ...
            JOIN data."user" student ON participant.user_id = student.id
            AND student.role = 'student'::data.user_role
            LEFT JOIN data.assignment_grade ON assignment_grade.assignment_submission_id = sub.id
    )
    SELECT
        assignment_slug, count(*) AS count, avg(points) AS average,
        min(points) AS min, max(points) AS max,
        max(points_possible) AS points_possible, stddev_pop(points) AS stddev,
        array_agg(points ORDER BY points) AS grades
    FROM included_scores
    GROUP BY assignment_slug
    HAVING count(*) >= 3
; ALTER VIEW api.assignment_grade_distributions
    OWNER TO yelukerest_migrator
;
--
...
CREATE TABLE data.assignment_grade_event (
    ...
    CONSTRAINT assignment_grade_event_points_in_range CHECK (points >= 0::double precision
    AND points <= points_possible::double precision),
    CONSTRAINT assignment_grade_event_points_possible_check CHECK (points_possible >= 0)
)
; ALTER TABLE data.assignment_grade_event
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.assignment_grade_events AS
    SELECT
        id, event_type, operation, assignment_submission_id, assignment_slug,
        points_possible, points, description, grade_created_at,
        grade_updated_at, created_at, created_by_user_id, source, reason,
        import_id
    FROM data.assignment_grade_event
; ALTER VIEW api.assignment_grade_events
    OWNER TO api
;
--
...
CREATE TABLE data.assignment_grade_exception (
    ...
    CONSTRAINT assignment_grade_exception_fractional_credit_check CHECK (fractional_credit >= 0::numeric
    AND fractional_credit <= 1::numeric),
...
; ALTER TABLE data.assignment_grade_exception
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.assignment_grade_exceptions AS
    SELECT
        id, assignment_slug, is_team, user_id, team_nickname, fractional_credit,
        closed_at, created_at, updated_at
    FROM data.assignment_grade_exception
; ALTER VIEW api.assignment_grade_exceptions
    OWNER TO api
;
--
...
CREATE VIEW api.assignment_grades AS
    SELECT
        assignment_slug, points_possible, assignment_submission_id, points,
        description, created_at, updated_at
    FROM data.assignment_grade
; ALTER VIEW api.assignment_grades
    OWNER TO api
;
--
...
CREATE VIEW api.assignment_submissions AS
    SELECT
        ...
        created_at, updated_at
    FROM data.assignment_submission
; ALTER VIEW api.assignment_submissions
    OWNER TO api
;
--
...
CREATE VIEW api.engagements AS
    SELECT user_id, meeting_slug, participation, created_at, updated_at
    FROM data.engagement
; ALTER VIEW api.engagements
    OWNER TO api
;
--
...
CREATE TABLE data.grade_event (
    ...
    CONSTRAINT grade_event_points_finite_nonnegative CHECK (points >= 0::double precision
    AND points <= 100000::double precision)
)
; ALTER TABLE data.grade_event
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.grade_events AS
    SELECT
        id, event_type, operation, snapshot_slug, user_id, points, description,
        grade_created_at, grade_updated_at, created_at, created_by_user_id,
        source, reason, import_id
    FROM data.grade_event
; ALTER VIEW api.grade_events
    OWNER TO api
;
--
...
CREATE TABLE data.grade (
    ...
    CONSTRAINT grade_points_finite_nonnegative CHECK (points >= 0::double precision
    AND points <= 100000::double precision),
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at)
)
; ALTER TABLE data.grade
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.grade_snapshot_distributions AS
    SELECT
        grade.snapshot_slug, count(grade.user_id) AS count,
        avg(grade.points) AS average, min(grade.points) AS min,
        max(grade.points) AS max, stddev_pop(grade.points) AS stddev,
...
; ALTER VIEW api.grade_snapshot_distributions
    OWNER TO yelukerest_migrator
;
--
...
CREATE TABLE data.grade_snapshot (
    ...
    CONSTRAINT grade_snapshot_slug_check CHECK (slug ~ '^[a-z0-9-]+$'::text
    AND char_length(slug) < 60),
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at)
)
; ALTER TABLE data.grade_snapshot
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.grade_snapshots AS
    SELECT slug, description, created_at, updated_at
    FROM data.grade_snapshot
; ALTER VIEW api.grade_snapshots
    OWNER TO api
;
--
...
CREATE VIEW api.grades AS
    SELECT points, snapshot_slug, user_id, description, created_at, updated_at
    FROM data.grade
; ALTER VIEW api.grades
    OWNER TO api
;
--
...
CREATE TABLE data.mcp_grant_revocation (
    ...
    CONSTRAINT mcp_grant_revocation_client_id_check CHECK (char_length(client_id) > 0
    AND char_length(client_id) <= 500),
...
; ALTER TABLE data.mcp_grant_revocation
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.mcp_grant_revocations AS
    SELECT id, user_id, netid, client_id, client_name, scopes, revoked_at
    FROM data.mcp_grant_revocation
; ALTER VIEW api.mcp_grant_revocations
    OWNER TO api
;
--
...
; ALTER TABLE data.mcp_jwt_mint_event
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.mcp_jwt_mint_anomalies AS
    SELECT
        caller_app_name, created_at AS window_end,
        created_at - '00:10:00'::interval AS window_start,
        (
            SELECT count(DISTINCT p.user_id) AS count
            FROM data.mcp_jwt_mint_event p
            WHERE
                p.caller_app_name = e.caller_app_name
                AND p.created_at > (e.created_at - '00:10:00'::interval)
...
; ALTER VIEW api.mcp_jwt_mint_anomalies
    OWNER TO api
;
--
...
CREATE TABLE data.meeting (
    ...
    CONSTRAINT meeting_slug_check CHECK (slug ~ '^[a-z0-9-]+$'::text
    AND char_length(slug) < 60),
...
CREATE VIEW api.meetings AS
    SELECT
        title, slug, summary, description, begins_at, duration, meeting_type,
        is_draft, created_at, updated_at
    FROM data.meeting
; ALTER VIEW api.meetings
    OWNER TO api
;
--
...
CREATE VIEW api.platform_version AS
    SELECT
        'yelukerest'::text AS platform, 1 AS platform_compatibility_version,
        3 AS schema_compatibility_version, 5 AS admin_api_version
...
CREATE VIEW api.quiz_grade_distributions AS
    SELECT
        quiz_grade.quiz_id, count(quiz_grade.user_id) AS count,
        avg(quiz_grade.points) AS average, min(quiz_grade.points) AS min,
...
; ALTER VIEW api.quiz_grade_distributions
    OWNER TO yelukerest_migrator
;
--
...
CREATE TABLE data.quiz_grade_event (
    ...
    CONSTRAINT quiz_grade_event_points_in_range CHECK (points >= 0::double precision
    AND points <= points_possible::double precision),
    CONSTRAINT quiz_grade_event_points_possible_check CHECK (points_possible >= 0)
)
; ALTER TABLE data.quiz_grade_event
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.quiz_grade_events AS
    SELECT
        id, event_type, operation, quiz_id, user_id, points, points_possible,
        description, grade_created_at, grade_updated_at, created_at,
        created_by_user_id, source, reason, import_id
    FROM data.quiz_grade_event
; ALTER VIEW api.quiz_grade_events
    OWNER TO api
;
--
...
CREATE TABLE data.quiz_grade_exception (
    ...
    created_at timestamp with time zone DEFAULT current_timestamp NOT NULL,
    updated_at timestamp with time zone DEFAULT current_timestamp NOT NULL,
    CONSTRAINT quiz_grade_exception_fractional_credit_check CHECK (fractional_credit >= 0::numeric
    AND fractional_credit <= 1::numeric),
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at)
)
; ALTER TABLE data.quiz_grade_exception
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.quiz_grade_exceptions AS
    SELECT
        id, quiz_id, user_id, fractional_credit, closed_at, created_at,
        updated_at
    FROM data.quiz_grade_exception
; ALTER VIEW api.quiz_grade_exceptions
    OWNER TO api
;
--
...
CREATE VIEW api.quiz_grades AS
    SELECT
        quiz_id, points, points_possible, description, user_id, created_at,
        updated_at
    FROM data.quiz_grade
; ALTER VIEW api.quiz_grades
    OWNER TO api
;
--
...
CREATE TABLE data.quiz_submission (
    quiz_id int NOT NULL,
    user_id int DEFAULT request.user_id() NOT NULL,
    created_at timestamp with time zone DEFAULT current_timestamp NOT NULL,
    updated_at timestamp with time zone DEFAULT current_timestamp NOT NULL,
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at)
)
; ALTER TABLE data.quiz_submission
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.quiz_submissions AS
    SELECT quiz_id, user_id, created_at, updated_at
    FROM data.quiz_submission
; ALTER VIEW api.quiz_submissions
    OWNER TO api
;
--
...
CREATE VIEW api.quizzes WITH (security_barrier=true) AS
    SELECT
        id, meeting_slug, points_possible, is_offline, is_draft, duration,
        open_at, closed_at, created_at, updated_at, is_draft = false
        AND open_at < current_timestamp
        AND current_timestamp < closed_at AS is_open
    FROM data.quiz
    WHERE request.user_role() = 'faculty'::text OR is_draft = false
; ALTER VIEW api.quizzes
    OWNER TO api
;
--
...
CREATE TABLE data.team (
    ...
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at),
    CONSTRAINT valid_team_nickname CHECK (nickname ~ E'^[\\w]{2,20}-[\\w]{2,20}$'::text)
)
; ALTER TABLE data.team
    OWNER TO yelukerest_migrator
;
--
...
CREATE TABLE data.ui_element (
    ...
    CONSTRAINT ui_element_key_check CHECK (key ~ E'^[a-z0-9\\-]+$'::text
    AND char_length(key) < 50),
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at)
)
; ALTER TABLE data.ui_element
    OWNER TO yelukerest_migrator
;
--
...
CREATE VIEW api.ui_elements AS
    SELECT key, body, is_markdown, created_at, updated_at
    FROM data.ui_element
; ALTER VIEW api.ui_elements
    OWNER TO api
;
--
...
CREATE TABLE data.user_secret (
    ...
    updated_at timestamp with time zone DEFAULT current_timestamp NOT NULL,
    CONSTRAINT updated_after_created CHECK (updated_at >= created_at),
    CONSTRAINT user_or_team CHECK ((team_nickname IS NOT NULL
    AND user_id IS NULL) OR (team_nickname IS NULL
    AND user_id IS NOT NULL)),
    CONSTRAINT user_secret_body_check CHECK (octet_length(body) <= 8192),
    CONSTRAINT user_secret_slug_check CHECK (slug ~ '^[a-z0-9][a-z0-9_-]+[a-z0-9]$'::text
    AND char_length(slug) < 100),
...
CREATE VIEW api.user_secrets AS
    SELECT
        id, slug, body, is_user_visible, user_id, team_nickname, created_at,
        updated_at
    FROM data.user_secret
; ALTER VIEW api.user_secrets
    OWNER TO api
;
--
...
--
ALTER TABLE data.artifact
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.artifact_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: assignment_field_submission_event_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.assignment_field_submission_event
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.assignment_field_submission_event_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: assignment_grade_event_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.assignment_grade_event
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.assignment_grade_event_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: assignment_grade_exception_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.assignment_grade_exception
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.assignment_grade_exception_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: assignment_submission_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.assignment_submission
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.assignment_submission_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: grade_event_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.grade_event
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.grade_event_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: mcp_grant_revocation_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.mcp_grant_revocation
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.mcp_grant_revocation_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: mcp_jwt_mint_event_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.mcp_jwt_mint_event
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.mcp_jwt_mint_event_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: quiz_grade_event_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.quiz_grade_event
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.quiz_grade_event_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: quiz_grade_exception_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.quiz_grade_exception
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.quiz_grade_exception_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: quiz_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.quiz
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.quiz_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: user_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data."user"
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.user_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
-- Name: user_secret_id_seq; Type: SEQUENCE; Schema: data; Owner: cluster_admin
--
ALTER TABLE data.user_secret
    ALTER id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME data.user_secret_id_seq START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1 )
;
--
...
--
CREATE UNIQUE INDEX assignment_grade_exception_unique_team
ON data.assignment_grade_exception USING btree (assignment_slug, team_nickname)
WHERE is_team = true
;
--
-- Name: assignment_grade_exception_unique_user; Type: INDEX; Schema: data; Owner: cluster_admin
--
CREATE UNIQUE INDEX assignment_grade_exception_unique_user
ON data.assignment_grade_exception USING btree (assignment_slug, user_id)
WHERE is_team = false
;
--
...
ON data.assignment_submission USING btree (team_nickname, assignment_slug)
...
ON data.assignment_submission USING btree (user_id, assignment_slug)
WHERE team_nickname IS NULL
;
--
-- Name: idx_artifact_quiz_id_fk; Type: INDEX; Schema: data; Owner: cluster_admin
--
CREATE INDEX idx_artifact_quiz_id_fk
ON data.artifact USING btree (quiz_id)
;
--
-- Name: idx_artifact_user_id_fk; Type: INDEX; Schema: data; Owner: cluster_admin
--
CREATE INDEX idx_artifact_user_id_fk
ON data.artifact USING btree (user_id)
;
--
...
ON data.assignment_field USING btree (assignment_slug)
;
--
...
ON data.assignment_field_submission_event USING btree (created_by_user_id)
;
--
...
ON data.assignment_field_submission_event USING btree (assignment_submission_id, assignment_field_slug, created_at, id)
;
--
...
ON data.assignment_field_submission USING btree (assignment_field_slug, assignment_slug, assignment_field_is_url, assignment_field_pattern)
;
--
...
ON data.assignment_field_submission USING btree (assignment_submission_id, assignment_slug)
;
--
...
ON data.assignment_field_submission USING btree (submitter_user_id)
;
--
...
ON data.assignment_grade USING btree (assignment_slug, points_possible)
;
--
...
ON data.assignment_grade_event USING btree (created_by_user_id)
;
--
...
ON data.assignment_grade_event USING btree (assignment_submission_id, created_at, id)
;
--
...
ON data.assignment_grade_exception USING btree (assignment_slug, is_team)
;
--
...
ON data.assignment_grade_exception USING btree (team_nickname)
;
--
...
ON data.assignment_grade_exception USING btree (user_id)
;
--
...
ON data.assignment_grade USING btree (assignment_submission_id, assignment_slug)
;
--
...
ON data.assignment_submission USING btree (assignment_slug, is_team)
;
--
...
ON data.assignment_submission_participant USING btree (user_id)
;
--
...
ON data.assignment_submission USING btree (submitter_user_id)
;
--
...
ON data.assignment_submission USING btree (team_nickname)
;
--
...
ON data.mcp_grant_revocation USING btree (user_id, client_id, revoked_at DESC)
;
--
...
ON data.mcp_jwt_mint_event USING btree (caller_app_name, created_at, user_id)
;
--
...
ON data.quiz_grade_event USING btree (quiz_id, user_id, created_at, id)
;
--
...
ON data.user_secret USING btree (user_id, slug)
WHERE team_nickname IS NULL
;
--
...
--
CREATE POLICY artifact_access_policy ON data.artifact TO api USING ((request.user_role() = ANY('{student,ta}'::text[])
AND is_user_visible
AND request.user_id() = user_id) OR request.user_role() = 'faculty'::text) WITH CHECK (request.user_role() = 'faculty'::text)
;
--
...
--
CREATE POLICY assignment_field_submission_insert_policy ON data.assignment_field_submission FOR INSERT TO api WITH CHECK (request.user_role() = 'faculty'::text OR (request.user_role() = ANY('{student,ta}'::text[])
...
--
CREATE POLICY assignment_field_submission_select_policy ON data.assignment_field_submission FOR SELECT TO api USING ((request.user_role() = ANY('{student,ta}'::text[])
AND (submitter_user_id = request.user_id() OR EXISTS (
    SELECT ass_sub.id
    FROM api.assignment_submissions ass_sub
    WHERE ass_sub.id = assignment_field_submission.assignment_submission_id
) OR data.assignment_field_submission_is_writable_by_current_user(assignment_submission_id))) OR request.user_role() = 'faculty'::text)
;
--
-- Name: assignment_field_submission assignment_field_submission_update_policy; Type: POLICY; Schema: data; Owner: cluster_admin
--
CREATE POLICY assignment_field_submission_update_policy ON data.assignment_field_submission FOR UPDATE TO api USING (request.user_role() = 'faculty'::text OR (request.user_role() = ANY('{student,ta}'::text[])
AND (submitter_user_id = request.user_id() OR data.assignment_field_submission_is_writable_by_current_user(assignment_submission_id)))) WITH CHECK (request.user_role() = 'faculty'::text OR (request.user_role() = ANY('{student,ta}'::text[])
AND submitter_user_id = request.user_id()
AND data.assignment_field_submission_is_writable_by_current_user(assignment_submission_id)))
;
--
...
--
CREATE POLICY assignment_grade_access_policy ON data.assignment_grade TO api USING ((request.user_role() = ANY('{student,ta}'::text[])
AND EXISTS (
    SELECT ass_sub.id
    FROM api.assignment_submissions ass_sub
    WHERE assignment_grade.assignment_submission_id = ass_sub.id
...
--
CREATE POLICY engagement_access_policy ON data.engagement TO api USING ((request.user_role() = 'student'::text
AND request.user_id() = user_id) OR request.user_role() = ANY('{faculty,ta}'::text[]))
;
--
...
--
CREATE POLICY grade_access_policy ON data.grade TO api USING ((request.user_role() = ANY('{student,ta}'::text[])
AND request.user_id() = user_id) OR request.user_role() = 'faculty'::text) WITH CHECK (request.user_role() = 'faculty'::text)
;
--
...
--
CREATE POLICY mcp_grant_revocation_access_policy ON data.mcp_grant_revocation TO api USING (request.user_role() = 'faculty'::text OR request.user_id() = user_id OR (request.user_role() = 'app'::text
AND request.app_name() = 'authapp'::text)) WITH CHECK ((request.user_role() = 'app'::text
AND request.app_name() = 'authapp'::text) OR request.user_id() = user_id OR request.user_role() = 'faculty'::text)
;
--
...
--
CREATE POLICY quiz_grade_access_policy ON data.quiz_grade TO api USING ((request.user_role() = ANY('{student,ta}'::text[])
AND request.user_id() = user_id) OR request.user_role() = 'faculty'::text) WITH CHECK (request.user_role() = 'faculty'::text)
;
--
...
--
CREATE POLICY quiz_grade_exception_access_policy ON data.quiz_grade_exception TO api USING ((request.user_role() = ANY('{student,ta}'::text[])
AND request.user_id() = user_id) OR request.user_role() = 'faculty'::text) WITH CHECK (request.user_role() = 'faculty'::text)
;
--
...
--
CREATE POLICY quiz_submission_access_policy ON data.quiz_submission TO api USING ((request.user_role() = ANY('{student,ta}'::text[])
AND request.user_id() = user_id) OR request.user_role() = 'faculty'::text) WITH CHECK (request.user_role() = 'faculty'::text)
;
--
...
--
CREATE POLICY user_access_policy ON data."user" TO api USING ((request.user_role() = 'student'::text
AND request.user_id() = id) OR request.user_role() = ANY('{faculty,ta}'::text[]) OR (request.user_role() = 'app'::text
AND request.app_name() = 'authapp'::text))
;
--
...
--
REVOKE ALL ON FUNCTION auth.sign_jwt(user_id int, role data.user_role) FROM public
; GRANT ALL ON FUNCTION auth.sign_jwt(user_id int, role data.user_role) TO api
; GRANT ALL ON FUNCTION auth.sign_jwt(user_id int, role data.user_role) TO student
; GRANT ALL ON FUNCTION auth.sign_jwt(user_id int, role data.user_role) TO ta
; GRANT ALL ON FUNCTION auth.sign_jwt(user_id int, role data.user_role) TO faculty
;
--
-- Name: TABLE "user"; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data."user" TO api
;
--
-- Name: TABLE user_jwts; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.user_jwts TO student
; GRANT select ON api.user_jwts TO ta
; GRANT select ON api.user_jwts TO faculty
;
--
...
--
REVOKE ALL ON FUNCTION auth.sign_mcp_user_jwt(user_id int, role data.user_role, netid text, scopes text, jti text) FROM public
;
--
-- Name: TABLE artifact; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.artifact TO api
;
--
-- Name: TABLE artifacts; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.artifacts TO student
; GRANT select ON api.artifacts TO ta
; GRANT select, insert, delete, update ON api.artifacts TO faculty
;
--
...
--
GRANT select, insert, delete, update ON data.assignment_field_submission TO api
;
--
...
; GRANT select, insert, update ON api.assignment_field_submissions TO ta
; GRANT select, insert, delete, update ON api.assignment_field_submissions TO faculty
;
--
-- Name: TABLE assignment; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.assignment TO api
;
--
-- Name: TABLE assignment_field; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.assignment_field TO api
;
--
-- Name: TABLE assignment_fields; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.assignment_fields TO student
; GRANT select ON api.assignment_fields TO ta
; GRANT select, insert, delete, update ON api.assignment_fields TO faculty
;
--
-- Name: TABLE assignment_grade; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.assignment_grade TO api
;
--
-- Name: TABLE assignment_submission; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.assignment_submission TO api
;
--
-- Name: TABLE assignment_submission_participant; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.assignment_submission_participant TO api
;
--
-- Name: TABLE assignment_grade_distributions; Type: ACL; Schema: api; Owner: cluster_admin
--
GRANT select ON api.assignment_grade_distributions TO student
; GRANT select ON api.assignment_grade_distributions TO ta
; GRANT select ON api.assignment_grade_distributions TO faculty
;
--
...
--
GRANT select, insert, delete, update ON data.assignment_grade_exception TO api
;
--
-- Name: TABLE assignment_grade_exceptions; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.assignment_grade_exceptions TO student
; GRANT select ON api.assignment_grade_exceptions TO ta
; GRANT select, insert, delete, update ON api.assignment_grade_exceptions TO faculty
;
--
-- Name: TABLE assignment_grades; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.assignment_grades TO student
; GRANT select ON api.assignment_grades TO ta
; GRANT select, insert, delete, update ON api.assignment_grades TO faculty
;
--
-- Name: TABLE assignment_submissions; Type: ACL; Schema: api; Owner: api
--
GRANT select, insert, delete ON api.assignment_submissions TO student
; GRANT select, insert, delete ON api.assignment_submissions TO ta
; GRANT select, insert, delete, update ON api.assignment_submissions TO faculty
;
--
...
; GRANT select, insert, delete, update ON api.assignments TO faculty
;
--
-- Name: TABLE engagement; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.engagement TO api
;
--
-- Name: TABLE engagements; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.engagements TO student
; GRANT select, insert, delete, update ON api.engagements TO faculty
; GRANT select, insert, delete, update ON api.engagements TO ta
;
--
...
--
GRANT select, insert, delete, update ON data.grade TO api
;
--
-- Name: TABLE grade_snapshot_distributions; Type: ACL; Schema: api; Owner: cluster_admin
--
GRANT select ON api.grade_snapshot_distributions TO student
; GRANT select ON api.grade_snapshot_distributions TO ta
; GRANT select ON api.grade_snapshot_distributions TO faculty
;
--
-- Name: TABLE grade_snapshot; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.grade_snapshot TO api
;
--
-- Name: TABLE grade_snapshots; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.grade_snapshots TO student
; GRANT select ON api.grade_snapshots TO ta
; GRANT select, insert, delete, update ON api.grade_snapshots TO faculty
;
--
-- Name: TABLE grades; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.grades TO student
; GRANT select ON api.grades TO ta
; GRANT select, insert, delete, update ON api.grades TO faculty
;
--
...
--
GRANT select ON api.mcp_grant_revocations TO student
; GRANT select ON api.mcp_grant_revocations TO ta
; GRANT select ON api.mcp_grant_revocations TO faculty
...
--
GRANT select, insert, delete, update ON data.meeting TO api
;
--
-- Name: TABLE meetings; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.meetings TO student
; GRANT select ON api.meetings TO ta
; GRANT select ON api.meetings TO anonymous
; GRANT select, insert, delete, update ON api.meetings TO faculty
;
--
-- Name: TABLE platform_version; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.platform_version TO anonymous
; GRANT select ON api.platform_version TO student
; GRANT select ON api.platform_version TO ta
; GRANT select ON api.platform_version TO faculty
; GRANT select ON api.platform_version TO app
;
--
-- Name: TABLE quiz_grade; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.quiz_grade TO api
;
--
-- Name: TABLE quiz_grade_distributions; Type: ACL; Schema: api; Owner: cluster_admin
--
GRANT select ON api.quiz_grade_distributions TO student
; GRANT select ON api.quiz_grade_distributions TO ta
; GRANT select ON api.quiz_grade_distributions TO faculty
;
--
...
--
GRANT select, insert, delete, update ON data.quiz_grade_exception TO api
;
--
-- Name: TABLE quiz_grade_exceptions; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.quiz_grade_exceptions TO student
; GRANT select ON api.quiz_grade_exceptions TO ta
; GRANT select, insert, delete, update ON api.quiz_grade_exceptions TO faculty
;
--
-- Name: TABLE quiz_grades; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.quiz_grades TO student
; GRANT select ON api.quiz_grades TO ta
; GRANT select, insert, delete, update ON api.quiz_grades TO faculty
;
--
-- Name: TABLE quiz_submission; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.quiz_submission TO api
;
--
-- Name: TABLE quiz_submissions; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.quiz_submissions TO student
; GRANT select ON api.quiz_submissions TO ta
; GRANT select, insert, delete, update ON api.quiz_submissions TO faculty
;
--
-- Name: TABLE quiz; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.quiz TO api
;
--
-- Name: TABLE quizzes; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.quizzes TO student
; GRANT select ON api.quizzes TO ta
; GRANT select, insert, delete, update ON api.quizzes TO faculty
;
--
-- Name: TABLE team; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.team TO api
;
--
-- Name: TABLE teams; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.teams TO student
; GRANT select ON api.teams TO ta
; GRANT select, insert, delete, update ON api.teams TO faculty
;
--
-- Name: TABLE ui_element; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.ui_element TO api
;
--
-- Name: TABLE ui_elements; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.ui_elements TO student
; GRANT select ON api.ui_elements TO ta
; GRANT select ON api.ui_elements TO anonymous
; GRANT select, insert, delete, update ON api.ui_elements TO faculty
;
--
-- Name: TABLE user_secret; Type: ACL; Schema: data; Owner: cluster_admin
--
GRANT select, insert, delete, update ON data.user_secret TO api
;
--
-- Name: TABLE user_secrets; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.user_secrets TO student
; GRANT select ON api.user_secrets TO ta
; GRANT select, insert, delete, update ON api.user_secrets TO faculty
;
--
-- Name: TABLE users; Type: ACL; Schema: api; Owner: api
--
GRANT select ON api.users TO student
; GRANT select ON api.users TO ta
; GRANT select ON api.users TO app
...
INSERT INTO settings.secrets (key, value)
VALUES
    ('jwt_lifetime', '3600'), ('jwt_issuer', 'yelukerest'),
    ('jwt_audience', 'yelukerest-postgrest'),
    ('jwt_mcp_audience', 'yelukerest-mcp'), ('auth.default-role', 'anonymous'),
    ('auth.data-schema', 'data'), ('auth.api-schema', 'api')
