...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'api.grade_snapshot_distributions must exist and be migrator-owned';
    END IF;
    ...
    -- data.quiz.is_offline was dropped by the roadmap-9-admin-api migration:
    -- quizzes are paper-only, so the flag had one possible value. The check that
    -- stood here asserted it existed, which stopped being true the moment that
    -- migration applied.
    --
    -- Editing this file does not break immutability. Zapadka's definition hash
    -- binds the manifest to deploy.sql only (Manifest::definition_sha256), and
    -- deliberately excludes verify.sql -- verification is an observation of
    -- current state, and current state is allowed to move forward.
    ...
    IF (
        ...
        AND (n.nspname, c.relname, a.attname) IN (
            ...
            ('data', 'quiz', 'id'),
            -- data.quiz_grade_exception was dropped by roadmap-9-admin-api: it
            -- had no readers, so a quiz extension recorded a decision and
            -- changed nothing a student could do. The count moves 7 -> 6 with it.
            ('data', 'user', 'id'),
            ('data', 'user_secret', 'id')
        )
    ) <> 6 THEN
