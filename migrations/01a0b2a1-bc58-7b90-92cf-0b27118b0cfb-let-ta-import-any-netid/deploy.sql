...
-- Let a TA import include TA and faculty netids (issue #392).
--
-- 01a0b1c5 refused a TA batch naming anyone but a student: not the TA
-- themself, another TA, or faculty. The owner's ruling of 2026-09-18 is that
-- a TA may grade themself. TAs sit the quizzes too and their pages come
-- through the same scan; refusing them meant faculty had to load those rows
-- by hand. So under a TA claim the target may now be a student, a TA or a
-- faculty member, in the function and in the quiz_importer row policies that
-- say it a second time. An observer is still refused: nobody sits a quiz as
-- one. Every other TA rule stays: a non-blank p_reason, a draft quiz that
-- reads as no quiz, record-once (a TA still cannot change any existing
-- grade, their own included), and the per-request attendance bound. The
-- ledger and the grade events name the actor, so a TA grading themself is
-- visible there.
-- ---------------------------------------------------------------------------
-- The row policies, with the target clause widened to staff
-- ---------------------------------------------------------------------------
-- As 01a0b1c5 wrote them (and 01a0b241 rewrote the engagement UPDATE), with
-- the EXISTS over data."user" admitting a student, TA or faculty target under
-- a TA claim rather than a student alone. The draft-quiz clause and the
-- attendance-only clause stay, so the policies still refuse an observer
-- target, a draft-quiz write and a grade change for a TA whatever a function
-- owned by quiz_importer does.
...
AND EXISTS (
    ...
    WHERE
        ...
        AND u.role IN ('student', 'ta', 'faculty')
)
AND EXISTS (
    SELECT 1
    FROM data.quiz q
...
;
-- ---------------------------------------------------------------------------
-- The import, with the target rule widened to staff
-- ---------------------------------------------------------------------------
-- The body 01a0b241 shipped, with the block that refused a TA batch naming a
-- non-student now refusing an observer alone, and nothing else changed.
-- CREATE OR REPLACE keeps the owner, quiz_importer, and the execute grants.
...
BEGIN
    ...
    -- A TA records grades for the people who sat the quiz: students, TAs
    -- and faculty, themself included (#392). An observer never sits one.
    -- Faculty may grade any user row, as before.
    IF caller_is_ta THEN
        SELECT string_agg(DISTINCT target.netid, ', '
            ORDER BY target.netid) INTO offenders
        FROM data."user" target
        WHERE target.role NOT IN ('student', 'ta', 'faculty')
            AND target.netid IN (
        ...
        IF offenders IS NOT NULL THEN
            RAISE insufficient_privilege USING MESSAGE = format(
                'import_quiz_results: a TA can only record grades for students, TAs and faculty, not for: %s', offenders);
        END IF;
    END IF;
