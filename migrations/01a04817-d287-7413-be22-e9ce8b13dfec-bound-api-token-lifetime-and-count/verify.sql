...
; SELECT
    1 / (
        SELECT (NOT has_function_privilege('public', 'api.create_user_api_token(text, text[], timestamp with time zone)', 'EXECUTE'))::int
    )
; SELECT
    1 / (
        SELECT has_function_privilege('student', 'api.create_user_api_token(text, text[], timestamp with time zone)', 'EXECUTE')::int
    )
; SELECT
    1 / (
        SELECT (count(*) = 0)::int
