-- Restore the pre-lock bodies from upsert-user-secrets, which is what a
-- revert should leave behind. They are deadlock-prone under concurrent
-- overlapping batches -- that is the state this migration corrected, and
-- reverting is asking for it back.
