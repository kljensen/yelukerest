...
SELECT
    1 / (
        ...
        WHERE
            n.nspname = 'data'
            AND p.proname = 'enforce_request_row_bound'
    )
; SELECT
    1 / (
        SELECT (count(*) = 1)::int
        ...
        WHERE
            n.nspname = 'data'
            AND p.proname = 'request_row_bound_default'
    )
; SELECT
    1 / (
        SELECT (count(*) = 1)::int
...
SELECT
    1 / (
        ...
        FROM pg_trigger tg
        WHERE
            tg.tgrelid = 'data.assignment_field_submission'::regclass
            AND NOT tg.tgisinternal
...
SELECT
    1 / (
        ...
        WHERE
            NOT tg.tgisinternal
            AND tg.tgfoid IN ('data.enforce_request_row_bound()'::regprocedure, 'data.enforce_single_assignment_submission()'::regprocedure)
            AND ((tg.tgtype & 1) <> 0 OR (-- FOR EACH ROW
            tg.tgtype & 2) <> 0 OR (-- BEFORE
            tg.tgoldtable IS NULL
            AND tg.tgnewtable IS NULL))
    )
