...
CREATE FUNCTION data.request_row_bound_default() RETURNS int IMMUTABLE LANGUAGE sql RETURN 64
; COMMENT ON FUNCTION data.request_row_bound_default() IS 'Rows one student or TA request may affect in one table, unless a trigger names a stricter bound. Issue #346.'
;
...
CREATE FUNCTION request.reset_row_bound_counters() RETURNS void VOLATILE LANGUAGE sql SET search_path TO pg_catalog, pg_temp BEGIN ATOMIC
    SELECT set_config('yeluke.rows_affected', '', true)
; END
; COMMENT ON FUNCTION request.reset_row_bound_counters() IS 'Start a fresh per-request row budget. Called by api.check_request_jwt at the top of every PostgREST request. Issue #346.'
;
...
CREATE FUNCTION data.enforce_request_row_bound() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, data, request, pg_temp AS $$
DECLARE
    acting_role text;
    row_limit integer;
    affected bigint;
...
; COMMENT ON FUNCTION data.enforce_request_row_bound() IS 'Statement-level bound on rows one student or TA request may affect in one table. Issue #346.'
;
...
CREATE FUNCTION data.enforce_single_assignment_submission() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, data, request, pg_temp AS $$
DECLARE
    acting_role text;
    parents bigint;
BEGIN
...
; COMMENT ON FUNCTION data.enforce_single_assignment_submission() IS 'One statement may only touch assignment field submissions of a single assignment submission, for student and TA credentials. Issue #346.'
;
...
CREATE TRIGGER tg_assignment_field_submission_row_bound_insert AFTER INSERT ON data.assignment_field_submission REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_request_row_bound()
; CREATE TRIGGER tg_assignment_field_submission_row_bound_update AFTER UPDATE ON data.assignment_field_submission REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_request_row_bound()
; CREATE TRIGGER tg_assignment_field_submission_row_bound_delete AFTER DELETE ON data.assignment_field_submission REFERENCING OLD TABLE bounded_old_rows EXECUTE FUNCTION data.enforce_request_row_bound()
; CREATE TRIGGER tg_assignment_field_submission_single_parent_insert AFTER INSERT ON data.assignment_field_submission REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_single_assignment_submission()
; CREATE TRIGGER tg_assignment_field_submission_single_parent_update AFTER UPDATE ON data.assignment_field_submission REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_single_assignment_submission()
; CREATE TRIGGER tg_assignment_field_submission_single_parent_delete AFTER DELETE ON data.assignment_field_submission REFERENCING OLD TABLE bounded_old_rows EXECUTE FUNCTION data.enforce_single_assignment_submission()
;
...
-- deleted at all.
CREATE TRIGGER tg_assignment_submission_row_bound_insert AFTER INSERT ON data.assignment_submission REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_request_row_bound('4')
; CREATE TRIGGER tg_assignment_submission_row_bound_update AFTER UPDATE ON data.assignment_submission REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_request_row_bound('4')
; CREATE TRIGGER tg_assignment_submission_row_bound_delete AFTER DELETE ON data.assignment_submission REFERENCING OLD TABLE bounded_old_rows EXECUTE FUNCTION data.enforce_request_row_bound('4')
;
...
CREATE TRIGGER tg_engagement_row_bound_insert AFTER INSERT ON data.engagement REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_request_row_bound()
; CREATE TRIGGER tg_engagement_row_bound_update AFTER UPDATE ON data.engagement REFERENCING NEW TABLE bounded_new_rows EXECUTE FUNCTION data.enforce_request_row_bound()
; CREATE TRIGGER tg_engagement_row_bound_delete AFTER DELETE ON data.engagement REFERENCING OLD TABLE bounded_old_rows EXECUTE FUNCTION data.enforce_request_row_bound()
;
