...
<form class="create" method="POST" action="{{.Action}}"><button type="submit">{{if .State}}Try again{{else}}Create{{end}}</button></form>
{{end}}
</div>
</li>
{{end}}
...
// repositoriesStudentsOnlyTemplate is what staff see: the page is a
// student's, and nothing of anyone's is read to render this.
var repositoriesStudentsOnlyTemplate = template.Must(template.New("repositories-staff").Parse(`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
...
<h1>Repositories</h1>
<p class="note">Signed in as {{.NetID}}</p>
<p>This page is for students: it is where they connect a GitHub account and create repositories from the course's templates. Templates are configured through <code>api.repository_templates</code>, and the repositories students have made are in <code>api.assignment_repositories</code>.</p>
</main>
</body>
</html>
`))

// repositoriesScript is static/repositories.js: Create by fetch on the
// click (only then, see the file comment), the row re-rendered from the
// JSON, and polling while copying. It lives in a file of its own so its
// one piece of arithmetic, the next poll delay against the deadline, has a
// Bun test (static/repositories.test.js).
//
//go:embed static/repositories.js
var repositoriesScript string
...
func (h *provisioningHandler) servePage(w http.ResponseWriter, r *http.Request) {
	...
	// Students only. Staff read the attempt table under a policy that
	// shows every row, so a page rendered for them would pick some
	// student's attempt as if it were theirs; and none of the actions here
	// are theirs to take. They get the one line and nothing is read.
	if info.Role != "student" {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		if err := repositoriesStudentsOnlyTemplate.Execute(w, struct{ NetID, Stylesheet string }{netID, oauthStylesheetPath}); err != nil {
