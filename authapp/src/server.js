...
async function getYelukeUserInfo(pool, netid) {
  ...
  try {
    const result = await pool.query(
      "SELECT id,netid,nickname,role,team_nickname FROM api.users WHERE netid = $1",
      [netid]
    );
...
async function validateYelukeUser(req, res, next) {
  ...
  logger.info(logMsg);
  logMsg = `Found CAS information in session: ${JSON.stringify(
    req.session.cas
  )}`;
  logger.info(logMsg);
  ...
  if (!userInfo.id || userInfo.netid !== netid) {
    try {
      logger.info(logMsg);
      destroyRequestSession(req);
      res.status(httpstatus.UNAUTHORIZED).send(logMsg);
      return;
    } catch (error) {
      throw error;
    }
...
router.get("/jwt", validateYelukeUser, (req, res) => {
  ...
  const { payload, jwt, error } = jwtForRequest(req);
  if (error) {
  ...
  let logMsg;
  logMsg = `Had session info as follows = ${JSON.stringify(
    req.session.yeluke
  )}`;
  logger.info(logMsg);
...
router.get("/me", validateYelukeUser, async (req, res) => {
  ...
  const { jwt, error } = jwtForRequest(req);
  if (error) {
    return;
  }
